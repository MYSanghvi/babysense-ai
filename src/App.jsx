...
// ─── Constants ────────────────────────────────────────────────────────
const STORAGE_KEY  = "babysense_log";    // offline queue only
const PROFILE_KEY  = "babysense_profile"; // offline queue only
const CHAT_KEY     = "babysense_chat";     // chat history
...
function exportCSV(log) {
  const headers = ["Date","Start Time","End Time","Label","Duration","Diaper Type","Nursing Side","Feeding Type","Note"];
  const fmt = (ts) => {
    const d = new Date(ts);
  ...
  const rows = log.map(e => {
    const start    = fmt(e.startTime);
    const endTime  = e.duration ? fmt(e.startTime + e.duration * 1000).time : "";
    const startTime = e.duration ? start.time : "";
    const dur      = e.duration ? formatDuration(e.duration) : "";
    const endOrOnly = e.duration ? endTime : start.time;
    const vals = [start.date, startTime, endOrOnly, e.label ?? "", dur, e.diaperType ?? "", e.side ?? "", e.feedingType ?? "", e.note ?? ""];
    ...
    <div style={{ minHeight: "100vh", background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{
        ...
        <div style={{ position: "relative", marginBottom: 8 }}>
          ...
          />
          <button onClick={() => setShowPass(s => !s)} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: t.textMuted, fontSize: 16, padding: 0 }}>
...
// ─── Server (Google Sheets) Sync ─────────────────────────────────────
let pendingSync = false;

async function pushToServer(log, profile) {
...
function TimerModal({ ev, t, onSave, onClose }) {
  ...
  const [feedType,    setFeedType]    = useState(null); // Nursing / Formula
  const [note,        setNote]        = useState("");
  ...
  function handleSave() {
    ...
    onSave({ startTime: start, duration, side, feedingType: feedType, note });
  }
        ...
        {isFeed && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              {["Nursing","Formula"].map(ft => (
                <button key={ft} onClick={() => setFeedType(ft)} style={{ flex: 1, padding: "10px 4px", borderRadius: 12, border: `2px solid ${feedType === ft ? ev.streak : t.border}`, background: feedType === ft ? `${ev.streak}18` : t.bg3, color: feedType === ft ? ev.streak : t.textMuted, fontWeight: "600", fontSize: 13, cursor: "pointer", fontFamily: "'Poppins', sans-serif" }}>{ft}</button>
              ))}
            </div>
...
function HomeScreen({ log, prediction, aiLoading, t, setActiveModal, onSaveEntry, profile }) {
  ...
  useEffect(() => {
    localStorage.setItem(CHAT_KEY, JSON.stringify(chatHistory)); // chat stays local
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      ...
      <div style={{ background: t.card, borderRadius: 20, padding: "20px 18px", border: `1px solid ${t.border}`, display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        {/* ── Sync Status Panel ── */}
        <div style={{ background: t.bg3, borderRadius: 16, padding: "16px", marginBottom: 16, border: `1px solid ${t.border}` }}>
          <div style={{ fontSize: 13, fontWeight: "700", color: t.text, marginBottom: 12 }}>
            <i className="fa-solid fa-cloud" style={{ marginRight: 8, color: t.accent }} />Google Sheets Sync
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: t.textMuted }}>Connection</span>
            <span style={{ fontSize: 12, fontWeight: "700", color: isOnline ? "#34A853" : "#FF6B6B" }}>
              <i className={`fa-solid fa-circle`} style={{ fontSize: 8, marginRight: 5 }} />
              {isOnline ? "Online" : "Offline"}
            </span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: t.textMuted }}>Sync Status</span>
            <span style={{ fontSize: 12, fontWeight: "700", color: syncStatus === "synced" ? "#34A853" : syncStatus === "syncing" ? t.accent : syncStatus === "offline" ? "#FF6B6B" : t.textMuted }}>
              {syncStatus === "syncing" && <i className="fa-solid fa-spinner fa-spin" style={{ marginRight: 5 }} />}
              {syncStatus === "synced" ? "✓ Synced" : syncStatus === "syncing" ? "Syncing..." : syncStatus === "offline" ? "Offline" : "Not configured"}
            </span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
            <span style={{ fontSize: 12, color: t.textMuted }}>Last Synced</span>
            <span style={{ fontSize: 12, color: t.textMuted }}>{lastSynced ? timeSince(lastSynced) : "Never"}</span>
          </div>
          {!SHEETS_URL && <div style={{ fontSize: 11, color: "#FFD93D", background: "#FFD93D18", borderRadius: 8, padding: "8px 10px", marginBottom: 8 }}>
            <i className="fa-solid fa-triangle-exclamation" style={{ marginRight: 6 }} />Set SHEETS_URL in App.jsx to enable sync
          </div>}
          {SHEETS_URL && (
            <button onClick={onManualSync} style={{ width: "100%", padding: "11px", borderRadius: 12, border: `1px solid #34A853`, background: "#34A85318", color: "#34A853", fontWeight: "700", fontSize: 13, cursor: "pointer", fontFamily: "'Poppins', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
...
export default function App() {
  ...
  // Server-first: push every log change to Google Sheets
  useEffect(() => {
    if (log.length === 0) return;
    localStorage.setItem(STORAGE_KEY, JSON.stringify(log)); // offline backup
    if (SHEETS_URL) {
      pushToServer(log, profile).then(ok => {
        setSyncStatus(ok ? "synced" : "offline");
        setLastSynced(prev => ok ? Date.now() : prev);
      });
    }
  }, [log]);
  useEffect(() => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "'Poppins', sans-serif", maxWidth: 420, margin: "0 auto" }}>
      ...
      {tab === "settings" && <SettingsScreen
              profile={profile}
              onSave={p => { setProfile(p); setDarkMode(p.theme === "light" ? false : p.theme === "dark" ? true : isNightTime()); }}
              darkMode={darkMode}
              ...
              onManualSync={() => {
                ...
                pushToServer(log, profile).then(ok => {
                  setSyncStatus(ok ? "synced" : "offline");
                  setLastSynced(prev => ok ? Date.now() : prev);
                  showToast(ok ? "Synced to Google Sheets!" : "Sync failed — check connection.");
                });
              }}
            />}
